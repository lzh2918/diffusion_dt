...
def evaluate(**deps):
    ...
    loadpath = "/data/user/liuzhihong/paper/big_model/diffusion/exp_result/decision_diffuser/origin/hopper-medium-expert-v2/24-0430-200152/checkpoint"
    ...
    while sum(dones) <  num_eval:
        ...
        action_list = []
        for i in range(10):
            samples = trainer.ema_model.conditional_sample(conditions, returns=returns) # 这一句产生traj的
            np_samples = to_np(samples)
            print("begin unsample")
            unnormalize_sample = dataset.normalizer.unnormalize(np_samples, "observations")
            # obs_comb = torch.cat([samples[:, 0, :], samples[:, 1, :]], dim=-1)
            obs_comb = torch.cat([samples[:, :-1, :], samples[:, 1:, :]], dim=-1)
