...
import sys
sys.path.append("/data/user/liuzhihong/paper/big_model/diffusion/decision_diffuser_collect_data")
...
class TrainConfig:
    ...
    num_workers: int = 4
    # evaluation params
    target_returns: Tuple[float, ...] = (2000.0, 2500.0, 3000.0)
    ...
    diffusion_data_load_path: str = "/data/user/liuzhihong/paper/big_model/diffusion/exp_result/decision_diffuser_collect_data/first_edition/hopper-medium-v2/horizon_20/24-0521-182911/hopper-medium-v2/24-0522-192105/save_traj.npy"
    ...
    def __iter__(self):
        while True:
            ...
            if np.random.rand(1)[0] > self.gen_per:
                ...
                yield self.traj_buffer.sample_one(self.reward_scale)
...
def train(config: TrainConfig):
    ...
    for i in range(10):
        ...
        batch = diffusion_databuffer.sample_batch(config.batch_size,0.001)
        states_diff, actions_diff, returns_diff, time_steps_diff, mask_diff = [b for b in batch]
        print("return_mean: ", len(returns), "  ",returns.mean()," ", returns.std())
        print("return_diff_mean: ", len(returns_diff),"  ",returns_diff.mean()," ", returns_diff.std())
        print("**************************")
