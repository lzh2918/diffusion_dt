...
class batch_eval_env:
    ...
    def reset(self):
        batch_data = []
        for single_eval_env in self.eval_env_list:
            # 最后的[0]用来降维,这里有没有可能有问题。
            batch_data.append(single_eval_env.reset()[0])
        batch_data = np.array(batch_data)
        self.zero_state = np.zeros_like(batch_data[0][None])
        self.last_done = [False for _ in range(self.eval_batch)]
        return batch_data
...
def eval_rollout(
    ...
    # 这里明显有点问题吧，好好检查一下
    time_steps = torch.arange(0, model.episode_len, dtype=torch.long, device=device).repeat(eval_batch).view(eval_batch,-1)
    ...
    episode_return, episode_len = np.zeros((eval_batch,)), np.zeros((eval_batch,))
    ...
    for step in range(model.episode_len):
        ...
        # check all done
        if done.all() == False and done.any() == True and if_uper_return == False:
            print("not all done")
        next_return[done] = 0
